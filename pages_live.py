...
/* LEFT_M/RIGHT_M (HUD + leaderboard gutters) and FS (font scale) are responsive,
   recomputed in resize(); COMPACT hides the desktop-only left panels on phones. */
var LEFT_M=340, RIGHT_M=260, PAD=0.05, FS=1, COMPACT=false, TINY=false;
function F(px){ return Math.round(px*FS); }   // scaled font size
...
function applyLayout(){
  // shrink HUD/leaderboard gutters + fonts so the track stays visible on phones
  TINY = W < 560; COMPACT = W < 880;        // COMPACT hides left weather/driver/legend
  if(TINY){            LEFT_M=6;   RIGHT_M=Math.round(Math.min(118, W*0.33)); FS=0.6; }
  else if(COMPACT){    LEFT_M=92;  RIGHT_M=168; FS=0.82; }
  else {               LEFT_M=340; RIGHT_M=260; FS=1; }
}
function resize(){
...
function draw(){
  ...
  if(noPos){
    ...
    ctx.textAlign='center'; ctx.textBaseline='top'; ctx.font='700 '+F(15)+'px Titillium Web,sans-serif';
    ctx.fillText(COMPACT ? ('GPS ended L'+frame.lap+' — timing only')
                 : ('GPS FEED ENDED ON LAP '+frame.lap+' (RED-FLAG PERIOD) — TIMING CONTINUES TO THE FLAG'),
                 LEFT_M+(W-LEFT_M-RIGHT_M)/2, COMPACT?F(70):34);
  }
...
function drawHUD(frame,t,st){
  ...
  ctx.fillStyle='#FFF'; ctx.font='700 '+F(24)+'px Titillium Web,sans-serif';
  ctx.fillText('LAP '+frame.lap+(META.total_laps?' / '+META.total_laps:''), pad, F(14));
  ctx.font=F(20)+'px Titillium Web,sans-serif';
  ctx.fillText(fmtTime(t), pad, F(14)+F(30));
  var stx=STATUS_TEXT[st];
  if(stx){ ctx.font='700 '+F(20)+'px Titillium Web,sans-serif'; ctx.fillStyle=stx[1]; ctx.fillText(stx[0],pad,F(14)+F(58)); }

  // session banner — centred over the track (short on phones, hidden on tiny)
  if(!TINY){
    ctx.textAlign='center'; ctx.fillStyle='#BBB'; ctx.font=F(13)+'px Titillium Web,sans-serif';
    var banner=COMPACT ? (META.event||'')
      : ((META.event||'')+' — '+(META.circuit||'')+', '+(META.country||'')+'  ·  '+META.year+' R'+META.round+'  ·  '+META.total_laps+' laps');
    ctx.fillText(banner, LEFT_M+(W-LEFT_M-RIGHT_M)/2, 10);
  }

  // left panels (weather · selected driver · controls legend) — desktop only
  if(!COMPACT){
    var wrow=null;
...
function drawLeaderboard(frame){
  ...
  rowLeaderboard(codes, COLORS, function(code){ return frame.d[code][3]; },
    function(code,i){ var c=frame.d[code];
      var out=(frame.lap-c[2]>=3);
      var gap = out?'OUT' : (i===0?'LEADER':('+'+(Math.abs(leaderDist-c[10])/10/55.56).toFixed(1)));
      return {tyre:c[4], pit:!!c[6], out:out, gap:gap}; });
}

// shared leaderboard renderer (replay + live) — responsive width/rows/fonts
function rowLeaderboard(codes, colors, posFn, infoFn){
  lbRects=[];
  var gut=TINY?4:(COMPACT?6:12);
  var x=Math.max(2, W-RIGHT_M+gut), w=RIGHT_M-gut-(COMPACT?2:8);
  var rowH=TINY?18:(COMPACT?20:25), top=COMPACT?26:30, head=COMPACT?34:38, first=COMPACT?52:58;
  var maxBottom=H-(COMPACT?64:130);                 // keep clear of transport buttons
  ctx.fillStyle='rgba(12,12,16,.82)';
  ctx.fillRect(x-gut, top, w+gut, Math.min(maxBottom-top, codes.length*rowH+(first-top)+6));
  ctx.fillStyle='#9B9BA8'; ctx.font='700 '+F(12)+'px Titillium Web,sans-serif'; ctx.textAlign='left'; ctx.textBaseline='top';
  ctx.fillText(TINY?'ORDER':'LEADERBOARD', x, head);
  codes.forEach(function(code,i){
    var y=first+i*rowH; if(y>maxBottom) return;
    var inf=infoFn(code,i), sel=selected.indexOf(code)>=0;
    if(sel){ ctx.fillStyle='rgba(225,6,0,.25)'; ctx.fillRect(x-4,y-3,w+2,rowH-2); }
    ctx.fillStyle='#888'; ctx.font=F(12)+'px JetBrains Mono,monospace'; ctx.textAlign='right';
    ctx.fillText(String(posFn(code)), x+F(16), y);
    ctx.fillStyle=colors[code]||'#FFF'; ctx.fillRect(x+F(20),y+1,3,F(12));
    ctx.fillStyle='#FFF'; ctx.font='700 '+F(13)+'px Titillium Web,sans-serif'; ctx.textAlign='left';
    ctx.fillText(code, x+F(26), y);
    var gx=x+w-2;
    if(!TINY && inf.tyre!=null && inf.tyre>=0){ ctx.beginPath(); ctx.arc(x+w*0.55,y+F(7),F(6),0,7); ctx.strokeStyle=TYRES[inf.tyre]; ctx.lineWidth=2.2; ctx.stroke(); }
    if(!TINY && inf.pit){ ctx.fillStyle='#FFB020'; ctx.font='700 '+F(10)+'px Titillium Web,sans-serif'; ctx.textAlign='left'; ctx.fillText('PIT', x+w*0.62, y+1); }
    ctx.fillStyle=inf.out?'#E10600':'#AAA'; ctx.font=F(12)+'px JetBrains Mono,monospace'; ctx.textAlign='right';
    var gap=inf.gap; if(TINY&&gap==='LEADER') gap='LDR';
    ctx.fillText(gap, gx, y);
    lbRects.push([code, x-gut, y-3, x+w, y+rowH-5]);
  });
}

function drawProgress(t){
  var left=LEFT_M, right=W-RIGHT_M, bottom=H-30, h=24, span=right-left;
  var total=frames[NF-1].t||1;
  // some races carry track-status events before lights-out (negative t, relative
  // to tmin) or after the last frame — clamp every marker to [0,total] so nothing
  // is drawn outside the bar (was leaking yellow chunks left/right of the bar)
  function cl(x){ return Math.max(0, Math.min(total, x)); }
  ctx.fillStyle='rgba(40,40,46,.9)'; ctx.fillRect(left,bottom-h,span,h);
  D.statuses.forEach(function(s){
    if(s[0]==='1') return;
    var s0=cl(s[1]), s1=cl(s[2]==null?total:s[2]);
    if(s1<=s0) return;
    var a=(s0/total)*span, b=(s1/total)*span;
  ...
  if(gmax>0&&gmax<total){
    var gx=left+(cl(gmax)/total)*span;
    ctx.fillStyle='rgba(0,0,0,.45)'; ctx.fillRect(gx,bottom-h,right-gx,h);
    ctx.fillStyle='#FFF'; ctx.fillRect(gx-1,bottom-h-3,2,h+6);
  }
  ctx.fillStyle='#E8E8E8'; ctx.fillRect(left, bottom-h, (cl(t)/total)*span, 3);
  ctx.strokeStyle='#555'; ctx.strokeRect(left,bottom-h,span,h);
}

function drawButtons(){
  buttons=[];
  // keep the controls centred over the track (not under the leaderboard)
  var cx=LEFT_M+(W-LEFT_M-RIGHT_M)/2;
  var bw=COMPACT?40:46, bh=COMPACT?30:34, gap=COMPACT?7:10, y=H-(COMPACT?56:100);
...
function liveHUD(codes){
  var pad=COMPACT?8:20;
  ctx.textAlign='left'; ctx.textBaseline='top';
  ctx.fillStyle='#FFF'; ctx.font='700 '+F(22)+'px Titillium Web,sans-serif';
  ctx.fillText('● LIVE', pad, F(14));
  ctx.font=F(13)+'px Titillium Web,sans-serif'; ctx.fillStyle='#BBB';
  ctx.fillText((LIVE.meta.circuit||'')+'  ·  '+(LIVE.meta.session||''), pad, F(14)+F(30));
  var stx=STATUS_TEXT[LIVE.flag];
  if(stx){ ctx.font='700 '+F(18)+'px Titillium Web,sans-serif'; ctx.fillStyle=stx[1]; ctx.fillText(stx[0],pad,F(14)+F(52)); }
  var w=LIVE.weather;
  if(w && !COMPACT){
    ctx.fillStyle='#9B9BA8'; ctx.font='12px Titillium Web,sans-serif';
    ctx.fillText('AIR '+(w.air_temperature!=null?w.air_temperature.toFixed(0):'–')+'°  TRACK '+(w.track_temperature!=null?w.track_temperature.toFixed(0):'–')+'°', 20, 118);
    ctx.fillText('HUM '+(w.humidity!=null?w.humidity.toFixed(0):'–')+'%   WIND '+(w.wind_speed!=null?w.wind_speed.toFixed(1):'–')+' m/s '+(w.wind_direction!=null?Math.round(w.wind_direction):'–')+'°', 20, 134);
    if(w.rainfall){ ctx.fillStyle='#3B9BFF'; ctx.fillText('RAINING', 20, 150); }
  }
  if(!COMPACT){
...
function liveLeaderboard(codes){
  rowLeaderboard(codes, LIVE.colors, function(code){ return LIVE.cars[code].pos||(codes.indexOf(code)+1); },
    function(code,i){ var c=LIVE.cars[code];
      var gap=(i===0)?'LEADER':(c.gap!=null?('+'+(typeof c.gap==='number'?c.gap.toFixed(1):c.gap)):'–');
      return {tyre:(c.tyre==null?-1:c.tyre), pit:false, out:false, gap:gap}; });
}
